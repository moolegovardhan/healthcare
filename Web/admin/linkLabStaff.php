...
$md = new MasterData();
$userData = $md->getNonActiveUsers('Lab');

if(count($userData) > 0){
    
    $diagnosticsData = $md->getLabData();
}
?>
<div class="col-md-12">
    <form action="../../Business/ActivateUser.php" method="POST">
    	<input type="hidden" name="from_page" value="diagonistics">
     ...
     <div  class="pull-right">
          <?php $count = count($userData);?>
             <input type="submit" value=" Attach Lab to Staff"class="btn-u btn-u-primary" id="activateUser"  <?php if($count < 1){ echo "disabled";}?>/>
     </div>
                ...
                <tr>
                    ...
                    <td>
                        <select name="<?php echo "hospital".$count;?>" class="form-control">
                             ...
                             <?php foreach($diagnosticsData as $data){?>
                                <option value="<?php echo $data->id;?>"><?php echo $data->diagnosticsname;?></option>
                             <?php } ?>  
                       </select>
