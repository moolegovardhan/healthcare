<?php
session_start();
include_once 'PregnancyTestsMasterData.php';
?>
<br/><br/><br/><br/>
...
<?php
$pd = new PregnancyTestsMasterData();
$patientid = $_POST['patientid'];//$_POST['patientid'];
$counter = $_POST['counter'];
for($i =1 ; $i<$counter+1; $i++){
    $paramdata = $_POST['textbox'.$i];
   $param = explode("$", $paramdata);
     $result = $pd->addTestsToMasterData($param[0], $param[1], $param[2], 'Y', $_SESSION['officeid']);
   //month+"$"+medicinename+"$"+observation;
     if($_POST['frommodule'] == ""){
        $message = "Data Updated Successfully";
        $url = "http://".$_SESSION['host']."/".$_SESSION['rootNode']."/Web/staff/staffindex.php?page=ptests";
